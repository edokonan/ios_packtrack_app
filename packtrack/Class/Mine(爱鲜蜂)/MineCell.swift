...
class MineCell: UITableViewCell {
    ...
    var mineModel: MineCellModel? {
        didSet {
            if let attrtitle = mineModel?.attrTitle{
...
//        arrowView.frame = CGRect(x: width - 20, y: (height - (arrowView.image?.size.height)!) * 0.5, width: (arrowView.image?.size.width)!, height: (arrowView.image?.size.height)!)
        ...
        titleLabel.frame = CGRect(x: iconImageView.frame.maxX + rightMargin, y: 0,
                                  width: width - (iconImageView.frame.maxX + rightMargin), height: height)
