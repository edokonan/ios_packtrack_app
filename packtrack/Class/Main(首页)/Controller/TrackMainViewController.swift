...
class TrackMainViewController: BaseViewController,UITableViewDataSource, UITableViewDelegate,
...
//    let trackStatusModel = TrackStatusModel.shared//model
    ...
    var RMdata:Results<RMTrackMain>?
...
//            ])
    ...
    func bind() {
        RMdata = IceCreamMng.shared.getTrackMainlistForMainView(self.nowSelectedStatus)
        Observable.array(from: RMdata!).subscribe(onNext: { (dogs) in
...
// MARK: bar menu
/*　BAR　Menu　*/
extension TrackMainViewController{
    func setBarItem(){
